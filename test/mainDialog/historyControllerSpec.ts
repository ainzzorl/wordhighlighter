...
describe('historyController', () => {
    ...
    beforeEach(() => {
        dao = {
            getDictionary: (callback: (dictionary: Array<DictionaryEntry>) => void) => {
                callback([]);
            },
    ...
    beforeEach(inject(($controller, $rootScope) => {
        $scope = $rootScope.$new();
        controller = $controller('historyController', { $scope: $scope, NgTableParams: NgTableParams, dao: dao });
    }));
    ...
    describe('refresh', ()  => {
        describe('valid interval', () => {
            beforeEach(() => {
                ...
                dao.getDictionary = (callback: (dictionary: Array<DictionaryEntry>) => void) => {
                    let result = [];
                ...
                dao.getHighlightingLog = (callback: (log: HighlightingLog) => void) => {
                    ...
                    entries.push(new HighlightingLogEntry('example3.com', dateDaysAgo(20), {
                        1: 10,
                        2: 10,
                        3: 10
                    }));
                    // Inside the interval.
                    entries.push(new HighlightingLogEntry('example2.com', dateDaysAgo(9), {
                        2: 60
                    }));
                    // Inside the interval.
                    entries.push(new HighlightingLogEntry('example1.com', dateDaysAgo(5), {
                        1: 1,
                        2: 2
                    }));
                    // Inside the interval.
                    entries.push(new HighlightingLogEntry('example1.com', dateDaysAgo(1), {
                        1: 10
                    }));
