...
angular
.module('mainDialog')
.controller('historyController', function($scope: any, NgTableParams: any, dao: DAO) {
    ...
    function populateTableData(dictionary: Array<DictionaryEntry>, highlightingLog: HighlightingLog) {
        ...
        for (let i = highlightingLog.entries.length - 1; i >= 0; i--) {
            ...
            let daysAgo = (now.getTime() - highlightingLogEntry.date.getTime()) / $scope.MILLISECONDS_IN_DAY;
            if (daysAgo > $scope.interval.value) {
                break;
            }
            Object.keys(highlightingLogEntry.highlights).forEach((dictionaryEntryIdStr) => {
